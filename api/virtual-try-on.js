...
export default async function handler(req, res) {
  ...
  try {
    ...
    const productImgUrl = product.img.startsWith('http') ? product.img : `${cdnBase}${product.img}`;

    const prodImgRes = await fetch(productImgUrl);
    if (!prodImgRes.ok) {
      return res.status(502).json({ error: 'Failed to retrieve garment reference image.' });
    }
    ...
    if (!vertexRes.ok) {
      ...
      if (vertexData.error?.message?.includes('billing')) {
        return res.status(503).json({
          error: 'Google Cloud billing is currently pending activation. Virtual try-on will be active shortly.'
        });
      }
      return res.status(vertexRes.status).json({
        error: vertexData.error?.message || 'Vertex AI Virtual Try-On failed.'
      });
    }
